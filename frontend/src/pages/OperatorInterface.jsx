...
function toFieldKey(label, index) {
  ...
  return knownKeys[label] || `field_${index + 1}`;
}
...
function createPlanFromPrompt(text) {
  ...
  const labels = extractFields(text);
  const fields = labels.length > 0 ? labels : ["名稱", "分類", "日期", "金額", "備註"];
...
export function OperatorInterface() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content: "請描述你想建立的資料庫工具。這是本機操作者介面，適合測試對話、待確認卡片與資料表操作流程。"
    }
  ]);
  ...
  function updateActivePlanFields(updater) {
    ...
    setPlans((prev) =>
      prev.map((plan) => (
        (plan.plan_id || plan.id) === planId
          ? { ...plan, schema: { ...plan.schema, fields: updater(plan.schema.fields) } }
          : plan
      ))
    );
    return true;
  ...
  function handleCommand(text) {
    ...
    if (text.includes("改成")) {
      ...
      const didUpdate = updateActivePlanFields((fields) =>
        fields.map((field, index) => field.display_name === fromName ? { ...createField(toName, index), data_type: field.data_type } : field)
      );
  ...
  async function renameField(toolId, fieldId, name) {
    setTools((prev) => prev.map((tool) => {
      ...
      return { ...tool, fields: tool.fields.map((field) => field.id === fieldId ? { ...field, display_name: name } : field) };
    }));
  }
    ...
    <main className="app-shell">
      ...
      <section className="chat-main">
        <header className="top-bar">
          <div className="brand-block operator-brand">
